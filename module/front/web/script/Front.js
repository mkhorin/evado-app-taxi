...
class Front {
    ...
    constructor () {
        ...
        this.on('click', '[data-action="newOrder"]', this.onNewOrder.bind(this));
        this.on('click', '[data-action="viewOrder"]', this.onViewOrder.bind(this));
        this.on('click', '[data-action="offer"]', this.onOffer.bind(this));
        this.on('click', '[data-action="offers"]', this.onOffers.bind(this));
        this.on('click', '[data-action="newOffer"]', this.onNewOffer.bind(this));
        this.on('click', '[data-action="waitingOrders"]', this.onWaitingOrders.bind(this));
        this.on('click', '[data-action="waitingOrder"]', this.onWaitingOrder.bind(this));
    ...
    onOrder (event) {
        event.preventDefault();
        const order = this.getActionTarget(event);
        this.trigger(this.getAuthAction('order'), {order});
    }
    ...
    onNewOrder (event) {
        event.preventDefault();
        this.trigger(this.getAuthAction('newOrder'));
    }

    onViewOrder (event) {
        event.preventDefault();
        const order = this.getActionTarget(event);
        this.trigger(this.getAuthAction('viewOrder'), {order});
    }

    onOffer (event) {
        event.preventDefault();
        const offer = this.getActionTarget(event);
        const parentHandler = this.getParentHandler(event.currentTarget);
        this.trigger(this.getAuthAction('offer'), {offer, parentHandler});
    }

    onOffers (event) {
        event.preventDefault();
        this.trigger(this.getAuthAction('offers'));
    }

    onNewOffer (event) {
        event.preventDefault();
        const order = this.getActionTarget(event);
        this.trigger(this.getAuthAction('newOffer'), {order});
    }

    onWaitingOrder (event) {
        event.preventDefault();
        const order = this.getActionTarget(event);
        this.trigger(this.getAuthAction('waitingOrder'), {order});
    }

    onWaitingOrders (event) {
        event.preventDefault();
        this.trigger(this.getAuthAction('waitingOrders'));
    }
}
